...
class ReminderReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        ReminderScheduler.rescheduleAfterFire(context, type)
    }
    ...
    private fun showNotification(context: Context, type: String) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(channelId, "תזכורות תזונה ושתייה", NotificationManager.IMPORTANCE_HIGH).apply {
        ...
        val pair = when(type) {
            ...
            else -> "💧 הגיע הזמן לשתות מים" to "שתה עכשיו מים וסמן אותם באפליקציה כדי להמשיך לעקוב."
        }
